...
class Contenedor {
    ...
    async getById(id) {
        try {
            let arrayDeElementos = (await db.collection(this.ruta).doc(id).get()).data()
            return arrayDeElementos
        } catch (err) {
            return { error: true, msg: err };
        }
    ...
    async deleteAll() {
        try {
            ...
            const buscandoProductoDbCarrito = (await db.collection(this.ruta).get()).docs
